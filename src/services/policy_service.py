...
#   例如：位于富春山健康城 → 环境约束权重 +0.05
#        位于桐庐经济开发区 → 基础设施权重 +0.05
#        产业为高碳排 → 环境约束权重 +0.10
...
POLICY_LIBRARY: dict[str, str] = {
    ...
    # --- 杭州市层 ---
    "hangzhou_15_5": "杭州市十五五规划纲要：桐庐为杭州都市圈西部生态经济核心区",
    "hangzhou_lifecycle": "杭州市工业用地全生命周期管理：标准地+亩均评价+达产复核+退出机制四位一体",
    "hangzhou_low_efficiency": "杭州市低效用地再开发专项行动方案：十五五目标较十四五翻倍",
    "hangzhou_m0": "杭州市M0新型产业用地管理办法：核心区位可申请，容积率2.0-4.0，配套比例≤30%",
    "hangzhou_iot": "杭州市智能物联产业生态圈建设方案：向桐庐延伸，快递物流智能装备为核心组件",
    # --- 桐庐县层 ---
    "tonglu_industrial_baseline": "桐庐县工业用地最低出让价：十一等 10.2万元/亩（国家规定最低价）",
    "tonglu_preference_logistics": "桐庐县供地偏好：快递物流装备——优先供地（最具识别度的产业标签）",
    "tonglu_preference_health": "桐庐县供地偏好：大健康（医疗器械/生物制造）——优先供地（富春山健康城专项指标）",
}
...
"""桐庐县各产业供地偏好矩阵（基于 tonglu_policy_research.md §五 供地偏好矩阵）。"""
...
def get_policy_refs(
    ...
    # 4. 乡镇特殊规则
    # TODO[BatchD]: 根据 town 细化（如"富春江镇"→强调健康城特殊政策）
    _ = town  # 当前未使用，留待后续批次
