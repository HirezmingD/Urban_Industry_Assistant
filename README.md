...
| 功能 | 版本 | 说明 |
|------|:--:|------|
| 🗺️ **卫星底图** | v1.0 | 天地图 WMTS 卫星影像，县域范围约束 |
| 📐 **框选查询** | v1.0 | Leaflet.Draw 框选 → 空间查询 → 实时显示网格数与总面积 |
| 🖱️ **单格悬停** | v1.0 | 鼠标移动 → 200ms 节流 → 单格高亮 |
| 📋 **点选标签** | v1.0 | 点击网格 → sticky note（4 方向避让 + 三角箭头） |
| 🏷️ **行政区划悬停** | v1.3 | 鼠标移入 → 乡镇边界高亮 + 名称 Tooltip |
| 🧠 **AI 评估** | v1.0 | 框选 + 对话 → 七维模型评分 + 产业推荐 + 政策依据 |
| 🏭 **企业匹配** | v1.0 | 多选企业 → 全域最优用地匹配（政府端） |
| 📊 **自进化面板** | v2.1 | 能力雷达图 + 进化曲线 + 经验胶囊列表 |
| 🎨 **智慧蓝 UI** | v2.2 | 15 CSS 变量双主题色彩系统，WCAG AA 通过 |
| 💬 **AI 回复卡片** | v2.3 | 结构化卡片渲染 + 一键最大化模式 |
| 🗺️ **对话地图联动** | v2.3 | 框选/视图网格硬编码推荐 → 地图高亮持久化 |
| 🎯 **企业择地匹配** | v2.4 | 独立引擎：企业画像 × 视口网格 → 多维匹配（面积/产业/设施 40/40/20）→ 分组着色 |
